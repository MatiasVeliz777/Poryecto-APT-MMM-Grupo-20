...
$query = $conn->prepare("SELECT id, DAY(fecha) AS day, fecha, titulo, hora, ubicacion FROM capacitaciones WHERE MONTH(fecha) = ? AND YEAR(fecha) = ? ORDER BY fecha ASC");
...
// Crear el HTML de cada tarjeta de capacitación con estilos y botones
foreach ($events as $day => $eventList) {
    foreach ($eventList as $event) {
        $eventCards .= "<div id='capacitacion-{$event['id']}' class='event-card clickeable' onclick=\"location.href='capacitacion_asistencia.php?capacitacion_id={$event['id']}'\" style='cursor: pointer; position: relative;'>
        ...
        // Si el usuario tiene permisos para modificar o eliminar capacitaciones
        if (isset($_SESSION['admin']) && $_SESSION['admin'] == 1) {
            // Botón para eliminar la capacitación
            ...
            // Botón para modificar la capacitación
        ...
        $capacitacion_id = $event['id'];
        $rut_usuario = $_SESSION['rut'];

        $check_sql = "SELECT fecha FROM capacitaciones WHERE id = ? AND fecha >= ?";
        $stmt_check = $conn->prepare($check_sql);
        $stmt_check->bind_param("is", $capacitacion_id, $fecha_actual);
        $stmt_check->execute();
        ...
        if ($result_check->num_rows > 0) {
            ...
            if ($result_check_asistencia->num_rows == 0) {
                // Mostrar el botón "Asistir" si el usuario no está registrado y la capacitación es futura
                $eventCards .= "<form method='POST' style='display: inline;' onclick='event.stopPropagation();'>
                                    <input type='hidden' name='capacitacion_id' value='{$event['id']}'>
        ...
        $eventCards .= "</div><hr>"; // Cerrar el div de la tarjeta de capacitación

        // Modal de actualización de capacitación
                    ...
                    <div class='modal-body'>
                        <form method='POST'>
                            ...
                            <div class='form-group mb-3'>
                                <label for='titulo-{$event['id']}'>Título de la Capacitación</label>
                            ...
                            <button type='submit' name='actualizar_capacitacion' class='btn btn-success'>Actualizar Capacitación</button>
                        </form>
                    </div>
                </div>
            </div>
