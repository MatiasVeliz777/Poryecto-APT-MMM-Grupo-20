...
// Capturar el año seleccionado o usar el año actual como predeterminado
$anio_actual = isset($_GET['year']) ? intval($_GET['year']) : date('Y');
...
// Consulta 1: Obtener todos los eventos del año seleccionado
$eventos_anuales_sql = "
...
// Si hay eventos en el año seleccionado, continuar con la segunda consulta
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    <div class="container mt-0">
        ...
        <div class="row justify-content-center mb-4">
            <div class="col-md-6">
                <form method="GET" action="" class="d-flex flex-wrap justify-content-center align-items-center bg-white p-3 shadow-sm rounded">
                    <label for="year" class="form-label me-2">Seleccione el año:</label>
                    <input type="number" name="year" id="year" class="form-control me-3" style="width: auto;" 
                           value="<?php echo $anio_actual; ?>" min="2000" max="2500" required>
                    <button type="submit" class="btn btn-primary">Ver Estadísticas</button>
                </form>
            </div>
        </div>

        <!-- Resumen de estadísticas y gráfico en la misma fila -->
...
<script>
    ...
    document.getElementById('downloadPDF').addEventListener('click', function () {
        html2canvas(document.getElementById('attendanceChart')).then(canvas => {
            ...
            pdf.text('Estadísticas Anuales de Asistencia a Eventos (<?php echo $anio_actual; ?>)', 10, 10);
            pdf.text(`Total de eventos en el año: <?php echo $total_eventos; ?>`, 10, 20);
            pdf.text(`Total de asistencias registradas: <?php echo $total_asistencias; ?>`, 10, 30);
            pdf.text(`Porcentaje de participación: <?php echo number_format($porcentaje_participacion, 2); ?>%`, 10, 40);

            pdf.addImage(imgData, 'PNG', x, y, imgWidth, imgHeight);
            pdf.save(`Estadisticas_Anuales_<?php echo $anio_actual; ?>.pdf`);
        });
    });
    </script>   
</div>
